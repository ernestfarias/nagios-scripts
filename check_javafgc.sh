...
INT_1=$(echo $CMD1 | awk '{split($0,a," "); print $15}')
...
#echo "RESULTADO3: $WARN  and $CRIT  and $TEST"
  ...
  if [ $RESULT -gt $CRIT ] ;then
    echo "CRITICAL - GarbageCollector-Events FGC=$RESULT | FGC(events)=$RESULT"
    exit $CRITICAL
  elif [ $RESULT -gt $WARN ]; then
    echo "WARNING - GarbageCollector-Events FGC=$RESULT | FGC(events)=$RESULT"
    exit $WARNING
  else
    echo "OK - GarbageCollector-Events FGC=$RESULT | FGC(events)=$RESULT"
    exit $OK
  fi
fi
