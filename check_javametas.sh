...
INT_1=$(echo $CMD1 | awk '{split($0,a," "); print $10}')
...
#WARN=
WARN=$(($RESULT1 + 10000000))
CRIT=$(($RESULT2 + 50000000))

#echo "RESULTADO3: $WARN  and $CRIT  and $TEST"

  if [ $RESULT2 -gt $CRIT ] ;then
    echo "CRITICAL - Metaspace Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2 | Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2"
    exit $CRITICAL
  elif [ $RESULT2 -gt $WARN ]; then
    echo "WARNING - Metaspace Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2 | Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2"
    exit $WARNING
  else
    echo "OK - Metaspace Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2 | Usage(Bytes)=$RESULT Capacity(Bytes)=$RESULT2"
    exit $OK
  fi
fi
