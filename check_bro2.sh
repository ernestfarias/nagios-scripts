...
# Set this to the proper location if your installation differs
CAPTURE_LOG=/mnt/data/bro/logs/current/capture_loss.log
STATS_LOG=/mnt/data/bro/logs/current/stats.log
...
This script should be run on the Bro manager.
  Options:
    ...
    -T <type>         Check type, "status/loss/capture_loss/myricom/print/droprec"
...
if [ $DROPRECCHECK -eq 1 ]; then
  ...
  INT_REC=$($BROCTL netstats | grep "$WORKERS" | sed 's/[a-z]*=//g' | awk '{ recvd += $3 ; dropped += $4 } END { printf("received=%d dropped=%d\n",recvd,dropped) }')
  REC=$(/usr/bin/printf "%d\n" $INT_REC 2>/dev/null)

  if [ $REC -gt $CRIT ] ;then
    echo "Average packet loss is: $INT_REC"
    exit $CRITICAL
  elif [ $REC -gt $WARN ]; then
    echo "Average packet loss is: $INT_REC"
    exit $WARNING
  else
    echo "Average packet loss is: $INT_REC"
