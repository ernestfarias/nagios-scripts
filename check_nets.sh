...
ipkts=`echo $output | awk '{print $4}'`
ierrs=`echo $output | awk '{print $5}'`
opkts=`echo $output | awk '{print $8}'`
oerrs=`echo $output | awk '{print $9}'`
idrop=`echo $output | awk '{print $6}'`
odrop=`echo $output | awk '{print $10}'`

if ( ( [ -n "$crit_ierrs" ] && [[ "$ierrs" -gt "$crit_ierrs" ]] ) || \
	( [ -n "$crit_oerrs"  ] && [[ "$oerrs" -gt "$crit_oerrs" ]] ) || \
	( [ -n "$crit_collis"  ] && [[ "$idrop" -gt "$crit_collis" ]]) || \
	( [ -n "$crit_queue"  ] && [[ "$idrop" -gt "$crit_queue" ]]) ); then
	msg="CRITICAL"
	status=2
else 
...
echo "netstat $msg - | ipkts=$ipkts ierrs=$ierrs;$warn_ierrs;$crit_ierrs opkts=$opkts oerrs=$oerrs;$warn_oerrs;$crit_oerrs idrop=$idrop;$warn_collis;$crit_collis odrop=$odrop;$warn_queue;$crit_queue"
