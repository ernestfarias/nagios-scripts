...
#IREC=$(/usr/bin/printf "%d\n" $INT_REC 2>/dev/null)
  ...
  if [ $CRITINT -gt $CRIT ] ;then
    echo "CRITICAL - | 'IO-ReadReq(req/s)'=$INT_1 IO-WriteReq(req/s)=$INT_2 QueueSize(#Req)=$INT_3"
    exit $CRITICAL
  elif [ $CRITINT -gt $WARN ]; then
    echo "WARNING - | 'IO-ReadReq(req/s)'=$INT_1 IO-WriteReq(req/s)=$INT_2 QueueSize(#Req)=$INT_3"
    exit $WARNING
  else
    echo "OK - | 'IO-ReadReq(req/s)'=$INT_1 IO-WriteReq(req/s)=$INT_2 QueueSize(#Req)=$INT_3" #aca $CMD1 aa $BROCTL"
    exit $OK
  fi
fi
